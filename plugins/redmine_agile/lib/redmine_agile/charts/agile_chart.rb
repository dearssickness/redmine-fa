...
#      jchart_fields_by_period = []
#      for item in chart_dates_by_period
#        jchart_fields_by_period << gregorian_to_jalali(item.strftime("%Y-%m-%d"))
#      end
#      jchart_fields_by_period
    ...
    def chart_field_by_date(date)
      jdate_list = gregorian_to_jalali(date.strftime("%Y-%m-%d")).split('-')
      case @interval_size
      when YEAR_INTERVAL
        jdate_list[0].to_i
      when QUARTER_INTERVAL, MONTH_INTERVAL
        month_name(jdate_list[1].to_i) + ' ' + jdate_list[0]
      else
        jdate_list[2].to_s + ' ' + month_name(jdate_list[1].to_i)
      end
#      case @interval_size
#      when YEAR_INTERVAL
#        date.year
