...
module ApplicationHelper
  ...
  def link_to_version(version, options = {})
    ...
    options = {:title => gregorian_to_jalali(format_date(version.effective_date))}.merge(options)
  ...
  def link_to_previous_month(year, month, options={})
    ...
    jdt = jdate_partials(target_year.to_s+'-'+target_month.to_s+'-'+ User.current.today.day.to_s)
    name = if jdt["month"] == 12
             "#{month_name(jdt["month"])} #{jdt["year"]}"
#             "#{month_name(target_month)} #{target_year}"
           else
             month_name(jdt["month"])
           end

    if @jdate_counter > 2 and @jdate_counter != nil
      @jdt_elements = jdate_elements(year, month, 1)
    else
      @jdt_elements = jdate_elements(year, month, User.current.today.day)
    end
...
#             "#{month_name(target_month)} #{target_year}"
  ...
  def time_tag(time)
    ...
    if @project
      link_to(text, project_activity_path(@project, :from => User.current.time_to_date(time)), :title => gregorian_to_jalali(time.strftime("%Y-%m-%d %H:%M:%S %:z")))
    else
      content_tag('abbr', text, :title => gregorian_to_jalali(time.strftime("%Y-%m-%d %H:%M:%S %:z")))
    end
  end
...
#    javascript_tag("kamaDatepicker('#{field_id}',{markToday: true, markHolidays: true, gotoToday: true, forceFarsiDigits: true});")
    javascript_tag("$('##{field_id}').persianDatepicker({format: 'YYYY/MM/DD', initialValue: false, altField: '.#{field_id}_hasdatepicker', altFormat: 'gregorian'});")
  end
  ...
  def jdate_elements(year, month, day)
    jdate = gregorian_to_jalali(year.to_s+'-'+month.to_s+'-'+day.to_s)
    ...
    jdt_elements["jcalendar_startdt"] = jdate_list[0]+'-'+jdate_list[1]+'-'+'1'
    jdt_elements["jcalendar_enddt"] =  jdate_list[0]+'-'+jdate_list[1]+'-'+jdt_elements["jnumber_of_days_in_month"].to_s
    jdt_elements["jfirst_day_of_month_cwday"] = jalali_to_gregorian(jdt_elements["jcalendar_startdt"]).to_date.cwday
    jdt_elements["jtog_calendar_startdt"] = jalali_to_gregorian(jdt_elements["jcalendar_startdt"]).to_date
    jdt_elements["jtog_calendar_enddt"] = jalali_to_gregorian(jdt_elements["jcalendar_enddt"]).to_date
    if @jdate_counter != nil
      @jdate_counter += 1 
    end
    jdt_elements
  end

  def jdate_partials(date)
    converted_date = gregorian_to_jalali(date)
    converted_date_list = converted_date.split('-')
    jdt = Hash.new
    jdt["year"] = converted_date_list[0].to_i
    jdt["month"] = converted_date_list[1].to_i
    jdt["day"] = converted_date_list[2].to_i
    if jdt["month"] > 9 && jdt["day"] > 11
      jdt["next_g_year"] = true
    else
      jdt["next_g_year"] = false
    end
    jdt
  end
end
