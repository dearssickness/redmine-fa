...
module IssuesHelper
  ...
  def render_issue_tooltip(issue)
    ...
    link_to_issue(issue) + "<br /><br />".html_safe +
      ...
      "<strong>#{@cached_label_status}</strong>: #{h(issue.status.name) + (" (#{gregorian_to_jalali(format_date(issue.closed_on))})" if issue.closed?)}<br />".html_safe +
      "<strong>#{@cached_label_start_date}</strong>: #{gregorian_to_jalali(format_date(issue.start_date))}<br />".html_safe +
      "<strong>#{@cached_label_due_date}</strong>: #{gregorian_to_jalali(format_date(issue.due_date))}<br />".html_safe +
      ...
      s <<
        content_tag(
          ...
          content_tag('td', check_box_tag("ids[]", child.id, false, :id => nil),
             ...
             content_tag('td', gregorian_to_jalali(format_date(child.start_date)), :class => 'start_date') +
             content_tag('td', gregorian_to_jalali(format_date(child.due_date)), :class => 'due_date') +
             content_tag('td',
      ...
      s <<
        content_tag(
          ...
          content_tag('td',
             ...
             content_tag('td', gregorian_to_jalali(format_date(other_issue.start_date)), :class => 'start_date') +
             content_tag('td', gregorian_to_jalali(format_date(other_issue.due_date)), :class => 'due_date') +
             content_tag('td',
  ...
  def email_issue_attributes(issue, user, html)
    ...
    %w(author status priority assigned_to category fixed_version start_date due_date).each do |attribute|
      if issue.disabled_core_fields.grep(/^#{attribute}(_id)?$/).empty?
				if (issue.send attribute).respond_to?(:strftime)
					attr_value = gregorian_to_jalali((issue.send attribute).strftime("%Y-%m-%d"))
				else
