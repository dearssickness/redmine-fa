...
#        @date_from = Date.civil(@year_from, @month_from, 1)
        jdate_from = Date.civil(@year_from, @month_from, 1)
        jdate_from = jdate_elements(jdate_from.year, jdate_from.month)
        @date_from = jdate_from["jtog_calendar_startdt"]
      ...
      def jdate_elements(year, month)
        if year.nil? and month.nil?
          jdate = gregorian_to_jalali(User.current.today.strftime("%Y-%m-%d"))
        else
          jdate = gregorian_to_jalali(year.to_s+'-'+month.to_s+'-'+'1')
        end
